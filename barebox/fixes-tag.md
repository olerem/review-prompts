...
- Assists the stable barebox team in determining which stable barebox versions should receive the fix
...
## Stable Barebox Considerations [FIXES-004]
...
3. **Backport Prerequisites**
   ...
   - Verify prerequisite commits are noted if present:
     ```
     Cc: <stable@vger.barebox.org> # 5.10.x: abc123: dependency description
     Cc: <stable@vger.barebox.org> # 5.10.x
     ```
