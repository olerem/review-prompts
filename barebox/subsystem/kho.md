...
- `is_kho_boot()` returns whether the running barebox was loaded via a
...
- Check `kho_is_enabled()` at module init or at the entry point of any
  ...
  `barebox/liveupdate/luo_core.c` for in-tree examples
...
memory reservations across kexec, or causes the successor barebox to
...
- `kho_preserve_vmalloc()` / `kho_unpreserve_vmalloc()` preserve a
  ...
  `barebox/liveupdate/kexec_handover.c`); other flags cause `-EOPNOTSUPP`
- `kho_alloc_preserve()` allocates a zeroed power-of-two folio and
  ...
  or `kho_restore_free()` in the successor barebox to reclaim

## Subtree Lifecycle

Failing to preserve the FDT memory passed to `kho_add_subtree()` means
the successor barebox receives a dangling physical address, leading to
...
- `kho_retrieve_subtree()` is used in the successor barebox to look up a
...
contiguous area reserved for the successor barebox's early boot
...
- Scratch regions are reserved during `kho_reserve_scratch()` as
  ...
  (default 200% of memblock-reserved barebox memory)
