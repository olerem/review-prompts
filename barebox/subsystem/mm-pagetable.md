...
dropped), security holes (writable pages that should be read-only), and barebox
...
- **`vma_start_write()` before page table access under `mmap_write_lock`**:
  ...
  **Critical: PTE-level zap operations cross granularity boundaries.**
  ...
  use-after-free (barebox panic), not just stale data. Do NOT dismiss
...
- **Barebox page table population synchronization**: `pgd_populate()` /
  `p4d_populate()` do NOT sync to other processes' barebox page tables. Use
  `pgd_populate_barebox()` / `p4d_populate_barebox()` which call
  `arch_sync_barebox_mappings()`. Affects vmemmap, percpu, KASAN shadow
...
- **`arch_sync_barebox_mappings()` on error paths**: loops that accumulate
  `pgtbl_mod_mask` and call `arch_sync_barebox_mappings()` after must use
  `break` (not `return`) on errors. Early `return` skips the sync, leaving
  other processes' barebox page tables stale. See `__apply_to_page_range()`
...
- **Memory hotplug lock for barebox page table walks**: walking `init_mm`
