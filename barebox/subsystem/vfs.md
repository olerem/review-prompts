...
inode triggers a `BUG_ON`, crashing the barebox. Confusing negative and
...
- **REF-walk**: Takes `d_lockref` references on dentries and per-CPU
  ...
  `-ELOOP`, `-ESTALE`).
- The barebox always attempts RCU-walk first, falling back to REF-walk on
...
- Mid-walk transition from RCU-walk to REF-walk is attempted via
  ...
  transition is one-way: once in REF-walk, the barebox never switches back
  to RCU-walk.
...
- `file->private_data` is initialized to `NULL` in `init_file()` and lives
  for the entire lifetime of the `struct file`. The barebox does NOT
