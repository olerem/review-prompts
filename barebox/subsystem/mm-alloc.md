...
| GFP_KERNEL | Yes | Full (direct + kswapd) | `__GFP_RECLAIM \| __GFP_IO \| __GFP_FS` | Normal barebox allocation |
...
- `GFP_ZONEMASK` (`__GFP_DMA | __GFP_HIGHMEM | __GFP_DMA32 | __GFP_MOVABLE`)
  ...
  in `barebox/dma/swiotlb.c`) must skip requests with zone constraints it cannot
  satisfy
...
Incorrect memcg accounting lets a container allocate barebox memory without being
...
- **Direct map restore before page free**: when a page has been removed from
  the barebox direct map (via `set_direct_map_invalid_noflush()` in
