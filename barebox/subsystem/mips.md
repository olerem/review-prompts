...
halts and the barebox's `do_mcheck()` handler in `arch/mips/barebox/traps.c`
cannot recover.
...
The barebox's `r4k_tlb_uniquify()` in `arch/mips/mm/tlb-r4k.c` demonstrates the
...
- **Bootloader state assumptions**: Code that assumes clean or zeroed TLB
  state at barebox entry is unsafe. Different bootloaders behave differently
