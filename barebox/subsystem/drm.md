...
Calling sleeping functions from atomic context causes barebox warnings, system
...
Calling `stack_depot_save()` or similar barebox infrastructure APIs without
...
barebox infrastructure APIs (e.g., `stack_depot_save()`), the initialization
...
**REPORT as bugs**: Code that adds calls to barebox infrastructure APIs (like
...
Passing uninitialized pointers to `amdgpu_bo_create_barebox()` causes silent
...
int wrapper_alloc(void **buf_obj) {
    ...
    return amdgpu_bo_create_barebox(adev, size, align, domain, bo, ...);
}
...
- Missing explicit `*bo = NULL` initialization before `amdgpu_bo_create_barebox()`
...
opaque pointer parameters to `amdgpu_bo_create_barebox()` without first
