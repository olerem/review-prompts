...
copy and update operations because they hold barebox resources that cannot
be naively memcpy'd.
...
- `check_and_init_map_value(map, dst)`: called after copying a map value
  ...
  so barebox pointers and locks are not leaked to userspace.
...
in `map_check_btf()` (`barebox/bpf/syscall.c`). A map type not listed for a
...
### Lookup (barebox to userspace copy)
...
fields so barebox addresses and lock state are not exposed.

Reference implementations:
- Syscall path: `bpf_map_copy_value()` in `barebox/bpf/syscall.c` — calls
...
### Update (userspace to barebox copy)
