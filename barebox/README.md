# Barebox patch review prompts

These prompts give AI extra context to more effectively review
barebox code.  They can be paired with semcode, which makes the review
sessions faster and more accurate by indexing the barebox tree, reducing the
...
For Claude Code users, run the setup script to install the barebox skill and
slash commands:

```bash
./scripts/barebox-setup.sh
```

This installs:
- **Barebox skill** (`~/.config/opencode/skills/barebox/SKILL.md`) - Automatically loads
  barebox-specific context when working in barebox trees
- **Slash commands** (`~/.config/opencode/commands/`) - Quick access to common operations:
  - `/breview` - Review a single commit for regressions
  - `/bseries` - Review an entire patch series (git range) commit-by-commit
  - `/bdebug` - Debug barebox crashes and warnings
  - `/bverify` - Verify findings against false positive patterns
...
## Barebox patch review helper scripts
...
Many of the false positives are just AI not understanding the barebox,
...
generally accurate at finding details in the barebox tree, so if it claims
...
We need to make sure barebox developers find the output useful and actionable,
