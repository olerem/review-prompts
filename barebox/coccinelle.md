...
transformations across the barebox tree. Prefer this approach over manual file
...
first. For example, `barebox/sched/*` must come before `barebox/*`, otherwise
sched files get claimed by the broader `barebox` group and the sched patch
...
the overlap problem where `barebox/sched/ext.c` matches both `barebox/sched/`
and `barebox/`.
...
file_to_subsystem() {
	...
	case "$f" in
		...
		# barebox/sched/*)  echo "sched" ;;
		# barebox/*)        echo "barebox" ;;
		*)                 echo "misc" ;;
	esac
}
