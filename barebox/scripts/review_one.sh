...
#   cd barebox ; semcode-index -s . --git base..last_sha

set -e

usage() {
    echo "usage: review_one.sh [--barebox <barebox_dir>] [--prompt <prompt_file>] [--series <end_sha>] [--working-dir <dir>] [--model <model>] [--append <string>] <sha>"
    echo "  --barebox: path to the base barebox directory (default: \$PWD/barebox)"
...
if [ -z "$BASE_BAREBOX" ]; then
    BASE_BAREBOX="$(pwd -P)/barebox"
fi
...
if [ ! -d "$BASE_BAREBOX" ]; then
    echo "Error: barebox directory does not exist: $BASE_BAREBOX" >&2
    exit 1
fi
...
if [ "$SHA" == "HEAD" ];
then
	SHA=$(cd $BASE_BAREBOX && git rev-parse --short HEAD)
fi
...
DIR="$BASE_BAREBOX.$SHA"

export TERM=xterm
export FORCE_COLOR=0

echo "Barebox directory: $BASE_BAREBOX" >&2
echo "Working directory: $WORKING_DIR" >&2
echo "Prompt file: $REVIEW_PROMPT" >&2
echo "Processing $SHA"

if [ ! -d "$DIR" ]; then
    (cd "$BASE_BAREBOX" && git worktree add -d "$DIR" "$SHA")
    while true; do
        if [ -d "$DIR" ]; then
            break
        fi
    ...
    if [ -d "$BASE_BAREBOX/.semcode.db" ]; then
        cp -al "$BASE_BAREBOX/.semcode.db" "$DIR/.semcode.db"
	HAVE_MCP=1
    else
        echo "Warning: $BASE_BAREBOX/.semcode.db not found, skipping MCP configuration" >&2
    fi
fi
