...
| `lore-reply` | Creates reply emails to patches on lore.barebox.org |

---

## create_changes.py

Extracts commit information and categorizes changes from a Barebox commit into a structured format suitable for parallel agent-based review.
...
```bash
ln -s /path/to/review-prompts/barebox/scripts/create_changes.py ~/.local/bin/create_changes.py
# or
cp /path/to/review-prompts/barebox/scripts/create_changes.py ~/.local/bin/
```
...
This script is designed to be called by the context-analyzer agent (see `barebox/agent/context.md`). The agent runs this script to prepare structured context files that other review agents can consume in parallel, with each agent reviewing a subset of FILE-N groups.
...
Creates properly formatted reply emails to patches posted on lore.barebox.org, with optional AI-assisted analysis of existing thread replies and patch verification.
...
1. Uses `b4 dig` to find the patch on lore.barebox.org by commit hash
...
- `b4` - For finding and downloading patches from lore.barebox.org
