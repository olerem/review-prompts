...
3. **Check for changes and commit if needed**:
   ...
   Look for:
   - **Modified files**: `modified: barebox/subsystem/*.md`
   - **New files**: `Untracked files: ... barebox/subsystem/*.md`

   If there are ANY changes (modified OR new files) in `barebox/subsystem/`:
   ```bash
   cd <prompt_dir>/.. && \
     git add -A barebox/subsystem/ && \
   ...
   - **New subsystem guide created**: e.g., `barebox/subsystem/drm.md` appears as untracked
   - **Existing guide updated**: e.g., `barebox/subsystem/btrfs.md` appears as modified
   - **Trigger table updated**: `barebox/subsystem/subsystem.md` modified to add new entry
