...
description: Analyzes a crash reproducer to identify barebox subsystems and operations
...
barebox subsystems it exercises, and how its operations relate to the
...
- Map every syscall/operation to the barebox code path it triggers.
- Identify the relationship between file descriptors, resources, and
  the crash function.
- Use semcode to look up barebox functions referenced by the reproducer
...
### Step 3: Map to Barebox Subsystems

For each unique barebox subsystem touched by the reproducer:

1. **Which device/file is involved?**
   - Device node paths -> driver module
   - Pseudo-filesystem paths (procfs/sysfs) -> barebox subsystem
   - Special file types -> owning subsystem

2. **What barebox functions handle each operation?**
...
each helper in the reproducer to understand what barebox operations it
